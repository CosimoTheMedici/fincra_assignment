...
} from "../src/utils/errors/ErrorHandlers";
...
describe("TicketService", () => {
  ...
  beforeEach(() => {
    ...
    ticketRepository.closeATicket = mockCloseTicket;
  });

  describe("createTicket", () => {
    it(" create a support ticket", async () => {
  ...
  describe("comment", () => {
    it("should add a comment to a ticket", async () => {
      ...
      const result = await ticketService.comment(
        userId,
        commentRequest,
        userType
      );
    ...
    it("Bad Request if user type is USER and no agent comments", async () => {
      const userId = "user123";
      const commentRequest = {
        ticketId: "ticket123",
        content: "Test comment",
    ...
    it("should throw Resource Not Found if ticket not found", async () => {
      const userId = "user123";
      const commentRequest = {
  ...
  describe("closeATicket", () => {
    it("should close a ticket successfully", async () => {
      ...
      const result = await ticketService.closeATicket(userId, ticketId);
    ...
    it("should throw BadRequest if ticket status is not IN_PROGRESS", async () => {
      const userId = "user123";
      const ticketId = "ticket123";
      const ticket = {
        _id: ticketId,
      ...
      try {
        await ticketService.closeATicket(userId, ticketId);
      } catch (error) {
        expect(error).toBeInstanceOf(BadRequest);
      }
    });
